...
{
    add_action('plugins_loaded', 'Nayax_init', 0);
    add_filter('woocommerce_payment_gateways', 'Nayax_add_gateway' );
}

function nayax_add_gateway( $methods ) { $methods[] = 'WC_Gateway_Nayax'; return $methods; }

function Nayax_init() {
  class WC_Gateway_Nayax extends WC_Payment_Gateway {
	  ...
	  public function __construct() {
		  ...
		  $this->id                = 'Nayax';
		  $this->icon              = apply_filters( 'woocommerce_Nayax_icon', plugin_dir_url('') . '/Nayax/Nayax_Logo.png' );
		  $this->has_fields        = false;
		  $this->order_button_text = __( 'Proceed to payment', 'woocommerce' );
		  $this->service_directUrl = 'https://nayaxtest176217138.wpcomstaging.com/';
		  $this->service_hppUrl    = 'https://uiservices.ecom.nayax.com/hosted/';
		  $this->method_title      = __( 'Nayax', 'woocommerce' );
		  $this->notify_url        = WC()->api_request_url( 'wc_gateway_Nayax' );
	  ...
	  public function admin_options() {
		  ...
		  ?>
		  <h3><?php _e( 'Nayax standard', 'woocommerce' ); ?></h3>
		  <p><?php _e( 'Nayax works by sending the user to base to enter their payment information.', 'woocommerce' ); ?></p>
	  ...
	  function init_form_fields() {
		  ...
		  $this->form_fields = array(
			  'enabled' => array(
				  ...
				  'type'    => 'checkbox',
				  'label'   => __( 'Enable Nayax standard', 'woocommerce' ),
				  'default' => 'yes'
			  ),
			  'title' => array(
			  ...
			  'description' => array(
				  ...
				  'default'     => __( 'Pay via Nayax; you can pay with your credit card if you don\'t have a Nayax account', 'woocommerce' )
			  ),
			  'merchantID' => array(
				  'title'       => __( 'Nayax Merchant Number', 'woocommerce' ),
				  'type'        => 'number',
				  'description' => __( 'Your merchant account number at Nayax.', 'woocommerce' ),
				  'default'     => '',
				  'desc_tip'    => true,
				  'placeholder' => '0000000'
			  ),
			  'securityKey' => array(
				  'title'       => __( 'Nayax Security Key', 'woocommerce' ),
				  'type'        => 'text',
			  ...
			  'paymentaction' => array(
				  'title'       => __( 'Nayax Action', 'woocommerce' ),
				  'type'        => 'select',
        ...
        'useHpp' => array(
				  ...
				  'type'        => 'checkbox',
				  'label'       => __( 'Use PaymentPage Redirection.', 'woocommerce' ),
    ...
    public function get_state( $cc, $state ) 
    {
      ...
      $send_total = number_format( $order->get_total(), 2, '.', '' );
          ...
          'trans_storePm'           => $this->useCCStorage ? '1' : '0',
    		  ...
    		  'signature'               => Nayax64_encode(md5( $this->merchantID . $send_total . get_woocommerce_currency() . $this->securityKey, true))
		  );
		  return $ret_args;
	  }
    ...
    function get_direct_args( $order ) {
	  ...
	  function receipt_page( $order_id ) {
		  echo '<p>' . __( 'Thank you - your order is now pending payment. You should be automatically redirected to Nayax to make payment.', 'woocommerce' ) . '</p>';
		  ...
		  wc_enqueue_js( '
			  $.blockUI({
					  message: "' . esc_js( __( 'Thank you for your order. We are now redirecting you to Nayax to make payment.', 'woocommerce' ) ) . '",
					  NayaxZ: 99999,
					  overlayCSS:
					  {
						  background: "#fff",
        ...
        '<form action="' . esc_url( $post_adr ) . '" method="post" id="payment_form" target="_top">
				  ...
				  <div class="payment_buttons">
					  <input type="submit" class="button alt" id="submit_payment_form" value="' . __( 'Pay via Nayax', 'woocommerce' ) . '" /> <a class="button cancel" href="' . esc_url( $order->get_cancel_order_url() ) . '">' . __( 'Cancel order &amp; restore cart', 'woocommerce' ) . '</a>
				  </div>
    ...
    {
        $order->add_order_note( 'Nayax ' . $modeText .  ' payment result :' . $replayCode);
	  ...
	  function process_payment( $order_id ) 
    {
        ...
        //print_r($params); echo '<br/>' . $response_array['body'];
        ...
        global $woocommerce;
        $woocommerce->add_error(__('Payment error:', 'woothemes') . ' (' . $r['Reply'] . ') ' . $r['ReplyDesc']);
        return;
		  } else {
      ...
      */
      ...
      if ($response_data && !empty( $response_data['replyCode'] ) ) {
        ...
        echo 'OK';
			  ...
			  wp_die( "Nayax notify handler Request Failure", "Nayax notify handler", array( 'response' => 200 ) );
		  }
	  }
